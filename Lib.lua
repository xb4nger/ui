...
do
    ...
    function BaseGroupboxFuncs:AddSlider(Idx, Info)
     ...
     function BaseGroupboxFuncs:AddProgressBar(Idx, Info)
    assert(Info.Text, string.format('AddProgressBar (IDX: %s): Missing `Text` string.', tostring(Idx)));
    ...
    local ProgressBar = {
        ...
        Visible = if typeof(Info.Visible) == "boolean" then Info.Visible else true;
        Text = Info.Text;
        ShowPercentage = if typeof(Info.ShowPercentage) == "boolean" then Info.ShowPercentage else true;
        Color = Info.Color or Library.AccentColor;
        Callback = Info.Callback or function(Value) end;
    };
    ...
    local ProgressText = Library:CreateLabel({
        Size = UDim2.new(1, 0, 1, 0);
        TextSize = 12;
        Text = '0%';
        ZIndex = 8;
    ...
    Library:AddToRegistry(ProgressInner, {
        BackgroundColor3 = 'MainColor';
        BorderColor3 = 'OutlineColor';
    });

    function ProgressBar:SetValue(Value)
        Value = math.clamp(Value or 0, 0, ProgressBar.Max);
        ProgressBar.Value = Value;
        
        local Percentage = (Value / ProgressBar.Max);
        local DisplayText = ProgressBar.ShowPercentage and math.floor(Percentage * 100) .. '%' or Value .. '/' .. ProgressBar.Max;
        ...
        Library:SafeCallback(ProgressBar.Callback, Value);
    end;
    ...
    function ProgressBar:SetVisible(Visibility)
        ProgressBar.Visible = Visibility;
        ProgressLabel.Visible = Visibility;
        ProgressOuter.Visible = Visibility;
        Groupbox:Resize();
    end;
